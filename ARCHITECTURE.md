...
- No verified runtime call from MyAgent into devTeam was found in the audited tree. `TODO: verify` whether any backend-to-backend link exists.
...
`devTeam` and `MyAgent` each carry their own `.git` directory; they are independent Forgejo repos checked out side-by-side rather than git submodules. Cloning `MyProject` alone will not pull them — clone each subproject separately when bootstrapping a fresh machine. `TODO: verify` whether a bootstrap script exists for that.
...
- **MyWeb** — Browser-only SPA shell exposing mail, news, search, chat, memory, calendar, admin, settings, devteam, whisper, and agent pages under one authenticated layout. See `MyWeb/docs/ARCHITECTURE.md` (`TODO: verify` exact path inside MyWeb).

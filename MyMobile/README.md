...
Your phone must be on the same LAN/VPN as the gateway because the prod backend at `https://myproject.thelunadog.com` is gated by `local-only@file` middleware in Traefik (see `services/traefik/dynamic.yml` in the homelab repo). WireGuard on `10.8.0.0/24` covers the away-from-home case.
